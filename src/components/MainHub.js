...
const MainHub = () => {
        ...
        return (
            <div className="container gap-4  p-5 border text-center my-5  border-air-blue bg-white shadow-lg shadow-air-blue rounded-lg">
                <div className="p-5">
                    ...
                    <div>
                        <button className=" font-lobster rounded-lg text-lg px-4 py-2 mt-5 tracking-wider bg-air-blue text-white outline-none"><Link to={`/mainhub/posting/${post._id}`}> View Details </Link> </button>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div >
            ...
            <div className="font-montserrat container grid gap-x-10 grid-cols-1 gap-4 mx-auto  py-9 px-10 sm:px-20 md:px-20  md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 justify-around ">{allPosting}</div>
        </div>
    )
}
