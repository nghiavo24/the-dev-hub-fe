...
function MainHub(){
    ...
    const getPostings =() => {
      ...
      const allPosting = posting.map((post) => {
        return (
          <div class="px-9 py-7 border-4 mx-5 my-5 border-gray-500">
                 <h3 class="text-2xl flex flex-wrap  text-blue-900" >{post.company}</h3>
                <h3 class="flex flex-wrap" >{post.title}</h3>
 
                <h3 class="flex flex-wrap" >{post.posted}</h3>
              <button class="rounded-lg bg-slate-500 text-lg mt-5 px-2 py-2 font-laonoto tracking-wider  hover:bg-indigo-500 outline-none"><Link to= {`/mainhub/posting/${post._id}`}  > View Details </Link> </button>
          </div>
        );
      });
